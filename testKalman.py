...
过程误差噪声Q和测量误差噪声R不是都应该是高斯白噪声吗？不一定
...
# initial guesses
x_hat[0] = 0.8  # initial value of predict
...
# P_minus是预测值(上一次的估计值)与真实值之间误差的协方差矩阵

# predict and update: the measurements are already done here(z).
for k in range(1, n_iter):
    # time update
    # X(k|k-1) = AX(k-1|k-1) + BU(k) + W(k): let A=1, BU(k)=0
    x_hat_minus[k] = x_hat[k - 1]
    P_minus[k] = P[k - 1] + Q  # P(k|k-1) = AP(k-1|k-1)A' + Q(k): let A=1

    # compute Kalman's gain
    # Kg(k)=P(k|k-1)H'/[HP(k|k-1)H' + R]: let H=1
    Kg[k] = P_minus[k] / (P_minus[k] + R)

    # measurement update
    # X(k|k) = X(k|k-1) + Kg(k)[Z(k) - HX(k|k-1)]: let H=1
    x_hat[k] = x_hat_minus[k] + Kg[k] * (z[k] - x_hat_minus[k])  # predict:残差(z[k]-xhatminus[k])
    # print('-- round %d estimate: %.3f' % (k, xhat[k]), flush=True)

    P[k] = (1 - Kg[k]) * P_minus[k]  # P(k|k) = (1 - Kg(k)H)P(k|k-1), H=1  # update

# 从概率论贝叶斯模型的观点来看前面预测的结果就是先验，测量出的结果就是后验。
plt.figure()
plt.plot(z, 'r+', label='noisy measurements')  # 测量值
plt.plot(x_hat, 'b-', label='kalman predicts')  # 过滤后的值
